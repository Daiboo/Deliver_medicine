  ...
  if(!(*(data_buf)==0xFF && *(data_buf+1)==0xFC))return; //不满足帧头条件
	...
	switch(target->task)
	{
		...
		case Tracking_task:  // 视觉任务
		{
			...
			int total_gray_bit = 8;

			// for (int i = 1; i <= 8; i++) {
			// 	if (target->x == 1 << i) {
			// 		gray_status[1]=i - (total_gray_bit / 2);
			// 		vision_status_worse/=2;
			// 		break;	
			// 	}
			// 	gray_status_backup[1][0];vision_status_worse++;
			// }
			...
			// 	case 0x0010:gray_status[1]=-7;	vision_status_worse/=2;break;				// 		0000 					//0000-0000-0001-0000b
