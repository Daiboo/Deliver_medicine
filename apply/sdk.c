...
{
	//小车硬件相关参数:编码器方向、电机驱动方向、轮子半径、舵机中值等  从eeprom读取设置
...
{
	pid_control_init(&seektrack_ctrl[0],//初始化灰度管循迹控制
	...
	pid_control_init(&seektrack_ctrl[1],// 初始化openmv循迹控制
	...
	pid_control_init(&steerangle_ctrl,//转向角度外环控制器结构体，用于控制偏航角
										5.0f,//比例参数
										0,	 //积分参数
										0,	 //微分参数
										50,  //偏差限幅值
	...
	pid_control_init(&steergyro_ctrl,//转向角速度内环控制器结构体
	...
	pid_control_init(&distance_ctrl,//距离控制结构体
										0.5,//比例参数
										0,//积分参数
										0,//微分参数
										75, //偏差限幅值
...
void steer_control(float *output)
{
	...
	{
		case ROTATE:  // 手动偏航控制模式
		{
			if(_flight_output->yaw_outer_control_output==0)//偏航控制杆置于中位,也就是不偏转
			{
...
void distance_control(void)
{
	...
	distance_ctrl.error=constrain_float(distance_ctrl.error,-60,60);
